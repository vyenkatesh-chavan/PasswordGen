            ...
            {showGenerator && (
              <div className="bg-gradient-to-br from-blue-50 to-indigo-50 p-5 rounded-xl space-y-4 border border-blue-100 animate-in fade-in slide-in-from-top-2 duration-300">
                ...
                <div className="space-y-3">
                  {["letters", "numbers", "symbols"].map((option) => (
                    <div
                      key={option}
                    ...
                    >
                      ...
                      <input
                        ...
                        type="number"
                        min="0"
                        value={(passwordOptions as any)[option]}
          ...
          {filteredEntries.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-400 text-sm mt-1">
                Add your first password entry above
              </p>
            </div>
          ) : (
                  ...
                  <div className="flex justify-between items-start gap-4">
                    <div className="flex-1 space-y-2">
                      <div className="flex items-start gap-2">
                        <span className="text-gray-500 font-medium text-sm min-w-16">
                          Site:
                        </span>
...
const initialForm: FormState = { siteName: "", link: "", password: "" };

export default function UserHomePage() {
  const router = useRouter();
  const params = useParams() as { userId?: string | string[] | undefined } | undefined;
  const userId = params?.userId;
  // ensure we always use a string when calling APIs
  const userIdStr = Array.isArray(userId) ? userId[0] : userId ?? "";
  ...
  // Redirect if no route userId (edge-case) or no session
  useEffect(() => {
    const storedUserId = sessionStorage.getItem("userId");
    ...
    if (!userIdStr) {
      // if route param missing, navigate to the authenticated user's page
      router.push(`/home/${storedUserId}`);
      return;
    }
  ...
  const fetchUserData = async (id: string) => {
    try {
      const res = await axios.get<VaultEntry[]>(`${process.env.NEXT_PUBLIC_API_URL}/vault/${id}`);
      setEntries(Array.isArray(res.data) ? res.data : []);
    } catch (err) {
      const error = err as AxiosError;
      console.error("Error fetching user data:", error?.response?.data ?? error.message ?? err);
  ...
  // Handle password generator option change
  const handleGeneratorChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const key = e.target.name as keyof PasswordOptions;
    const val = Number(e.target.value || 0);
    setPasswordOptions((prev) => ({ ...prev, [key]: val }));
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/save`, { userId: userIdStr, ...form });
      setMessage("✅ Data saved successfully!");
      setForm(initialForm);
      // use the string-safe id
      fetchUserData(userIdStr);
    } catch (err) {
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-white shadow-xl rounded-2xl p-8 border border-gray-100">
          ...
          {filteredEntries.length === 0 ? (
            ...
            <ul className="space-y-4">
              {filteredEntries.map((item) => (
                <li key={item._id} className="bg-gradient-to-br from-gray-50 to-blue-50 rounded-xl p-5 border border-gray-200 hover:border-blue-300 transition-all duration-200 hover:shadow-md">
                  <div className="flex justify-between items-start gap-4">
                    <div className="flex-1 space-y-2">
                      <div className="flex items-start gap-2">
                      ...
                      <div className="flex items-start gap-2">
                        ...
                        <a href={item.link} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:text-blue-700 underline break-all transition-colors">
                          {item.link}
                        </a>
                      </div>
                    ...
                    <button onClick={() => handleDelete(item._id)} className="bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-white px-4 py-2 rounded-lg font-medium transition-all duration-200 shadow-md hover:shadow-lg transform hover:-translate-y-0.5 whitespace-nowrap">
                      Delete
                    </button>
                  </div>
                </li>
